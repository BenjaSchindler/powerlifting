...
- `uv run pl prs`, `uv run pl e1rm <lift>` for each main lift,
  `uv run pl status`
- Read every session YAML of the block and pain-log entries dated inside it.

## 2. Write the review (in chat, honest and short)
...
- Pain timeline: what appeared, what resolved, what correlates with which
  exercise/volume (check `weekly_tonnage` via a quick
  `uv run python -c ...` if needed).
- Plan adherence: skipped/modified sessions and why.

## 3. Persist the conclusions
